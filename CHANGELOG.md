...
- Each build carries its version and a source fingerprint: in Help, in a `fmp-camera-sim-version` meta tag on the page and in the offline file, and in exported projects' `app` field, which previously always read "FMP Camera Simulator v1".
- `npm run verify:camera-sim-release` checks the log's format and the embedded stamp; with `--base` it also requires a new entry whenever `camera-sim/` changes. The simulator's pull-request workflow runs it against the PR's base, and the Pages deploy checks the format and stamp.

## 2026-09-23 — FMP Camera Simulator social card

- The simulator page gets its own Open Graph image (`camera-sim/og.png`, 1200 × 630) instead of borrowing the FMP suite card: a real frame from the simulator at the catwalk pose with the title band, generated by `scripts/make_camera_sim_og.mjs` from the committed build. `og:image`, its width, height and alt text, and `twitter:image` now point at it.
...
- Marked the processor port count as a generic minimum and the default cabinet watts and port capacity as sample planning values. Moved results ahead of the long input stack on mobile, added section jumps, and rolled the AV offline cache with Stage 3D's version pin.
...
* Release review follow-up restores account-owned checked-in and pending drafts after reauthentication, adds same-position saved-session selection, and makes lead corrections editable with preserved reasons and stable retry IDs. The no-JavaScript/loading state now carries commissioning and recovery guidance. Google sign-in styling is allowed through stylesheet-element CSP without allowing inline scripts.
...
* Added Guided Setup modal focus entry, Tab trapping, and trigger restoration so its keyboard behavior matches the existing Quick Switcher contract.
* Closed the site inconsistency audit with exact release, rendered journey, offline, interaction, crawl, and remaining manual-check evidence.
