...
var SUITE_SCHEMA='system-by-dave.av-suite.v1';
function isSuitePayload(parsed){if(!parsed||typeof parsed!=='object'||Array.isArray(parsed)) return false;if(parsed.schema!==undefined) return parsed.schema===SUITE_SCHEMA;return ['showName','venue','showDate','operator','phase','favorites','readiness','toolNotes'].some(function(key){return Object.prototype.hasOwnProperty.call(parsed,key);});}
function suiteHasWork(){return !!(state.showName||state.venue||state.operator||Object.keys(state.readiness||{}).length||Object.keys(state.toolNotes||{}).length);}
function importSuiteFile(file){var reader=new FileReader();reader.onload=function(){try{var parsed=JSON.parse(String(reader.result||''));if(!isSuitePayload(parsed)) throw new Error('Invalid payload');if(suiteHasWork()&&!window.confirm('Replace the current show'+(state.showName?' \u201c'+state.showName+'\u201d':'')+' with the imported suite JSON?')){showHint('Import cancelled. Current show kept.');return;}var previous=suitePayload();applySuitePayload(parsed);saveState();renderAll();showHint('Suite JSON imported.','',function(){applySuitePayload(previous);saveState();renderAll();showHint('Import undone. Previous show restored.');});}catch(e){showHint(importErrorMessage(e,'Suite JSON file'),'error');}};reader.onerror=function(){showHint('Could not read that suite JSON file.','error');};reader.readAsText(file);}
