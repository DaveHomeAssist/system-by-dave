...
export function cameraShell({ draft, position, registry, pages, view, testOnly, publicRelease = false, appHome = './', connectionText, dotClass, hasAlert }) {
  ...
  const statusBar = `<button class="status-bar ${hasAlert ? 'attention' : ''}" type="button" id="statusDetails"><span>${hasAlert ? 'Attention needed' : draft.pendingAction ? 'Pending · not submitted' : 'Draft stays on this device'}</span><span>View status ↗</span></button>`;
  return `<a class="skip" href="#screenTitle">Skip to current task</a><div class="shell">
    <header class="topbar"><a class="brand" href="${home}" aria-label="${homeLabel}"><span class="brand-mark">FMP</span><span>CAMERA<span class="brand-sub">OPERATIONS</span></span></a>${publicRelease ? '<a class="system-home" href="https://systembydave.com/" aria-label="System by Dave home" title="System by Dave home">SBD</a>' + themeToggle('top') : ''}<label class="position-select"><span>Physical position</span><select id="positionPicker" ${draft.checkedInReceipt ? 'disabled' : ''}>${registry.positions.map(item => `<option value="${esc(item.key)}" ${item.key === draft.positionKey ? 'selected' : ''}>${esc(item.displayName)}</option>`).join('')}</select></label><button id="showStatus" class="connection ${hasAlert ? 'attention' : ''}" aria-label="Open connection and draft status"><span class="dot ${dotClass}"></span><span>${hasAlert ? 'Attention' : identityLabel(connectionText)}</span></button></header>
