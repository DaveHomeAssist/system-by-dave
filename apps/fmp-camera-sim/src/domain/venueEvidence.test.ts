...
describe("per-property evidence", () => {
  ...
  it("does not let a photo-confirmed orientation settle an unknown heading", () => {
    ...
    expect(unsettledVenueItems(venue)).toEqual(["Pan-zero heading", "Bowl geometry", "Venue structures", "Lawn terrain"]);
    venue.mount.headingEvidence.status = "measured";
    expect(unsettledVenueItems(venue)).toEqual(["Bowl geometry", "Venue structures", "Lawn terrain"]);
  });
