...
async function main() {
  ...
  try {
    ...
    await delay(180);
    const plannerNav = await evaluate(`(() => { const rail=document.querySelector('.rail').getBoundingClientRect(); const ids=['stage3dLink','plannerFieldVerify']; const links=[...document.querySelectorAll('.rail-nav .nav-home,.rail-nav .nav-suite,.rail-nav .nav-stage,#plannerFieldVerify')].map(node=>{const rect=node.getBoundingClientRect();return{id:node.id||node.className,text:node.textContent.trim(),left:rect.left,top:rect.top,right:rect.right,bottom:rect.bottom,height:rect.height};});return{clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,rail:{left:rail.left,top:rail.top,right:rail.right,bottom:rail.bottom},links,ids:ids.map(id=>Boolean(document.getElementById(id)))};})()`);
    check('390px planner keeps Home, AV Suite, Stage 3D, and Field Verify in its two-row rail', plannerNav.ids.every(Boolean) && plannerNav.links.length === 4 && plannerNav.links.every(item => item.left >= plannerNav.rail.left - 0.5 && item.right <= plannerNav.rail.right + 0.5 && item.top >= plannerNav.rail.top - 0.5 && item.bottom <= plannerNav.rail.bottom + 0.5 && item.height >= 44) && plannerNav.scrollWidth <= plannerNav.clientWidth + 1, plannerNav);
    await cdp('Emulation.setDeviceMetricsOverride', { width: 1280, height: 900, deviceScaleFactor: 1, mobile: false });
