...
if (packageJson?.scripts?.['test:throwline-browser'] !== 'node scripts/probe_throwline_stage3d.js --no-sandbox && node scripts/probe_throwline_stage3d.js --no-sandbox --no-webgl') fail('Throwline browser verification must hard-fail WebGL and degraded-renderer probe runs.');
...
['setup', 'place', 'room', 'deliver'].forEach((name) => {
  ...
  requireMatch(stage, new RegExp(`role=["']tab["'][^>]*aria-controls=["']workflowPanel${title}["'][^>]*data-workflow-tab=["']${name}["']`), `Stage 3D workflow deck is missing the ${title} tab.`);
  requireMatch(stage, new RegExp(`id=["']workflowPanel${title}["'][^>]*role=["']tabpanel["'][^>]*data-workflow-panel=["']${name}["']`), `Stage 3D workflow deck is missing the ${title} panel.`);
});
['flightUnit', 'flightProvenance', 'flightScreen', 'flightSet', 'workflowBack', 'workflowNext', 'workflowPosition'].forEach((id) => {
  requireMatch(stage, new RegExp(`id=["']${id}["']`), `Stage 3D workflow deck is missing ${id}.`);
});
requireMatch(stage, /const WORKFLOW_STAGES = \['setup','place','room','deliver'\]/, 'Stage 3D must keep the workflow order explicit.');
requireMatch(stage, /function\s+selectWorkflowPanel\s*\([\s\S]*?aria-selected[\s\S]*?panel\.hidden[\s\S]*?workflowBack\.disabled[\s\S]*?workflowNext\.disabled/, 'Stage 3D workflow selection must synchronize tabs, panels, and navigation controls.');
requireMatch(stage, /ArrowRight[\s\S]*?ArrowLeft[\s\S]*?Home[\s\S]*?End/, 'Stage 3D workflow tabs must support arrow, Home, and End keys.');
requireMatch(stage, /\.workflow-panel\{[^}]*overflow:auto/, 'Stage 3D must keep scrolling inside the active workflow panel.');
requireMatch(stage, /\.workflow-panel\[hidden\]\{display:none\}/, 'Stage 3D must remove inactive workflow panels from layout and traversal.');
